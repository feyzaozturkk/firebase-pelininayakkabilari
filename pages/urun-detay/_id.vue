<template>
  <v-container v-if="dataSource!==undefined" class="fill-height about">
    <div class="d-flex flex-row ">
      ...
      <v-card height="530" width="1000px" color="white" class="ml-2 mt-0">
        ...
        <v-card height="360" width="1000px" color="#FFFFFF" class="ml-0 mt-0">
          <v-list-item three-line>
            <v-list-item-content>
              <v-list-item-subtitle class="ml-5 mt-2"> ₺{{ dataSource.fiyat / 10 }} 'den başlayan taksitlerle
              </v-list-item-subtitle>
              <v-list-item class="font-weight-bold ml-5 mt-5 "> {{ dataSource.urunAciklama }}
              </v-list-item>
              <div class="d-flex flex-row">
          ...
          <div class="d-flex flex-row">
            ...
            <v-btn @click.prevent="sepeteEkle(dataSource.id)" class="rounded-0 ml-12 mt-7"
                   style="background-color:#FF80AB; color:white" width="200px"> SEPETE
              EKLE
            </v-btn>
...
export default {
  ...
  methods: {
    ...
    sepeteEkle(id) {
      ...
      if (cookieData) {
        ...
        let count = 0;
        let totalPrice = 0;
        let i = cookieData[0].products.findIndex(it => it.id === id)
        if (i) {
          cookieData[0].products.push(this.dataSource)
        }
        cookieData[0].count = cookieData[0].count + 1;
        cookieData[0].totalPrice = cookieData[0].totalPrice + this.dataSource.fiyat;
      } else {
        cookieData = []
        cookieData.push({products: [], count: 1, totalPrice: this.dataSource.fiyat})
        cookieData[0].products.push(this.dataSource)
      }
  ...
  async asyncData({params, store}) {
    let dataSource = store.getters.getDataSource.find(item => item.id === params.id)
    return {dataSource}
  }
}
</script>
